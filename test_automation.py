...
def _resolve_path(p: str | None) -> str | None:
    ...
    script_dir_candidate = (Path(__file__).resolve().parent / path).resolve()
...
def run_test():
    ...
    if args.output:
        ...
        # Auto-generate output filename to avoid file lock issues (OneDrive, Excel open, etc.)
        excel_path = Path(args.excel) if args.excel else None
        if excel_path:
            args.output = str(excel_path.parent / (excel_path.stem + "_results" + excel_path.suffix))
        else:
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    try:
                        action_locator.click(timeout=10000)
                    except Exception:
                        # Re-locate the button in case the DOM changed
                        action_locator = page.get_by_role("button", name=re.compile(r"Transliterate", re.IGNORECASE)).first
                        action_locator.click(timeout=15000)
